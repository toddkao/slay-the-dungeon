...
export const RenderRewardsModal = observer(({onClickProceed} : {onClickProceed: () => void}) => {
  ...
  const renderCardReward = (reward: IReward) => {
    return (
      <RewardItem>
        ...
        <RewardItemDetails onClick={() => setSelectedReward(reward)}>
          ...
          <Column align="center">
            ...
            <Row style={{ gap: 50 }}>
              {selectedReward?.cards?.map((card) => (
                <RenderCard
                  cardState={card(uniqueId())}
                  onClick={() => {
                    player.addCardToDeck(card(uniqueId()));
                    setRewards(
                      rewards.filter(
